<template>
  <div>
    ...
    <div class="ui-item" v-if='value.show == 1'>
      <label class="ed-set-label">是否显示导航图标</label>
      <el-tooltip style="margin-left: 10px;">
        <el-switch
          v-model="value3"
          on-value="1"
          off-value="0" @change='changeIcon'>
        </el-switch>
      </el-tooltip>
    </div>
    <div class="ui-item" v-if="value.show == 1 && value.showIcon == 1">
...
export default{
  ...
  methods:{
   change(val) {
      ...
      if(value==1){
        this.$store.commit('updateTxt',{index,name,'setObject':{'cId':this.options[0].cId}})
        this.value2 = this.options[0].cId
      }
    },
    handler (val) {
      var cid = val
    ...
    choosemyTab() {
      ...
      // console.log("sssss",this.$store.state.Coms[cindex])
      ...
      this.$msgbox({
        ...
        message: h(txChoose, {
          props: {
            ...
            temp:this.temp,
            value:this.$store.state.Coms[cindex].items.tshow
          },
          on: {
            set: this.setHandler
          }
